...
def GenerateTags(TemplateCode, LandCategoryCode):
	...
	#only ask about London opportunities and constraints if site in London - only if TitlePostcodeFormatted exists
...
if response.status_code == 200:
	...
	if TitlePostcode is not None:
		...
		print("\nFor your information, we couldn't find any Title Postcode on PipeDrive, so we couldn't gauge metrics such as the closest main road or where the site is exactly. The site tag and letter will generate below as usual.")
else:
	sys.exit("Couldn't find this site in PipeDrive.")
