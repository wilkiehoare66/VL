...
TitleNumbers = input("Enter Title Numbers - they should be comma-space separated (for example: AB123456, CDF1234): ")
TitleNumbersFormatted = TitleNumbers.split(',')
TitleNumberArray = [TitleNumber.strip() for TitleNumber in TitleNumbersFormatted]
...
while True:
	...
	if len(LandCategoryArray) != len(TitleNumberArray):
		print("\nThe number of land categories you have provided do not match the number of title numbers provided. Please try again.\n")
...
for TitleNumber, LandCategoryCode in zip(TitleNumberArray, LandCategoryCodes):
	...
	if response.status_code == 200:
		...
		if TitlePostcode is not None and TitlePostcode != "":
			...
			print(f"\nFor your information, we couldn't find any Title Postcode on SearchLand for the title number {TitleNumber}, so we couldn't gauge metrics such as the closest main road or where the site is exactly, as well as failing to pinpoint its LPA and relevant local opportunities. The site tag and letter will generate below as usual.")
	else:
		sys.exit(f"Couldn't find this title on SearchLand (title number {TitleNumber}).")
		...
		if response.status_code == 200:
			...
			if response.status_code == 200:
				...
				if response.status_code == 200:
					...
					print(f"\nSlightly odd (probably contact WH), but we couldn't find the LPA from its ID on PipeDrive for the title number {TitleNumber}, so we couldn't examine any local opportunities. The site tag and letter will generate below as usual.")
			else:
				print(f"\nFor your information, we couldn't find the postcode's LPA on PipeDrive for the title number {TitleNumber}, so we couldn't examine any local opportunities. The site tag and letter will generate below as usual.")
		else:
			print(f"\nFor your information, we couldn't find this postcode on Postcodes.io for the title number {TitleNumber}, so we couldn't examine any local opportunities. The site tag and letter will generate below as usual.")
...
for TitleNumber, CodeFormatted in zip(TitleNumberArray, SiteTagArray):
	
	#Aware below two API sections are a duplicate of the other for loop but this was the most pain-free way I could see of listing all tags, followed by all letters (instead of 1-1, 2-2, etc.)
